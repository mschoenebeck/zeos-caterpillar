...
The multi-threaded build also emits `wasm_pkg_mt/snippets/wasm-bindgen-rayon-<hash>/src/workerHelpers.no-bundler.js`. This file is generated by `wasm-bindgen-rayon` (not hand-written) and is required at runtime by `zeos_caterpillar.js`. The `<hash>` folder name changes when `wasm-bindgen-rayon` or `wasm-bindgen` is upgraded - always copy the entire `wasm_pkg_mt/` directory.

To publish the wasm artifacts into the Cloak dApp repository:

```
make -j copy-dapp
```

By default this syncs into `../public/` (`wasm_pkg_st/`, `wasm_pkg_mt/` including `snippets/`, and `mint.params.b64`). Override the destination with `DAPP_PUBLIC=/path/to/dapp/public make copy-dapp`.
