...
impl Circuit<crate::engine::Scalar> for Spend {
    fn synthesize<CS: ConstraintSystem<crate::engine::Scalar>>(
        ...
        // Compute note a's symbol as a linear combination of the same bits
        ...
        // Plain spends are only valid for fungible tokens. Prove symbol != 0
...
#[cfg(test)]
#[path = "spend_soundness_tests.rs"]
mod spend_soundness_tests;
...
mod tests {
    ...
    use crate::engine::{fq_to_engine_scalar, Bls12};
    ...
    fn test_spend_circuit() {
        ...
        assert_eq!(
            cs.get_input(3, "symbol commitment/input variable")
                .to_repr(),
            scm.to_repr()
        );
        assert_eq!(
