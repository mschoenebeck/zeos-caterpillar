//! The [BLAKE2s] hash function with personalization support with seven rounds only.
//!
//! [BLAKE2s]: https://tools.ietf.org/html/rfc7693

use bellman::gadgets::{boolean::Boolean, multieq::MultiEq, uint32::UInt32};
...
#[allow(clippy::many_single_char_names)]
fn blake2s7r_compression<Scalar: PrimeField, CS: ConstraintSystem<Scalar>>(
    mut cs: CS,
    h: &mut [UInt32],
    m: &[UInt32],
    t: u64,
    ...
    */
...

    {
        ...
        // only 7 rounds instead of the usual 10
        for i in 0..7 {
...
pub fn blake2s7r<Scalar: PrimeField, CS: ConstraintSystem<Scalar>>(
    mut cs: CS,
    input: &[Boolean],
    personalization: &[u8],
    ...
    for (i, block) in blocks[0..blocks.len() - 1].iter().enumerate() {
        ...
        blake2s7r_compression(cs, &mut h, block, ((i as u64) + 1) * 64, false)?;
    }

    {
        let cs = cs.namespace(|| "final block");

        blake2s7r_compression(
            cs,
            &mut h,
            &blocks[blocks.len() - 1],
            (input.len() / 8) as u64,
...
mod test {
    ...
    fn test_blank_hash() {
        ...
        let input_bits = vec![];
        let out = blake2s7r(&mut cs, &input_bits, b"12345678").unwrap();
        assert!(cs.is_satisfied());
        assert_eq!(cs.num_constraints(), 0);

        // blank hash for blake2s7r
        let expected = hex!("5f498b5a48f35ac196750ee10a4076fbe9d37157b03649aad426d4c6a1035310");
    ...
    fn test_blake2s7r_constraints() {
        ...
        let input_bits: Vec<_> = (0..512)
            ...
            .collect();
        blake2s7r(&mut cs, &input_bits, b"12345678").unwrap();
        assert!(cs.is_satisfied());
        assert_eq!(cs.num_constraints(), 15216);
    }

    #[test]
    fn test_blake2s7r_precomp_constraints() {
    ...
    #[test]
    fn test_blake2s7r_constant_constraints() {
        let mut cs = TestConstraintSystem::<Scalar>::new();
        let mut rng = XorShiftRng::from_seed([
            0x59, 0x62, 0xbe, 0x5d, 0x76, 0x3d, 0x31, 0x8d, 0x17, 0xdb, 0x37, 0x32, 0x54, 0x06,
            0xbc, 0xe5,
    ...
    fn test_blake2s7r() {
        ...
        for input_len in (0..32).chain((32..256).filter(|a| a % 8 == 0)) {
            ...
            let r = blake2s7r(&mut cs, &input_bits, b"12345678").unwrap();
    ...
    fn test_blake2s7r_256_vars() {
        ...
        let expected = hex!("362215da919104ccbc728cbda275d4be642b6e80761cdea273ec56e3406fd55e");
    ...
    fn test_blake2s7r_700_vars() {
        ...
        let expected = hex!("da0c0745e31c2abf07ed24c3be0a5f3f37d5f882e45372e316fa7a79308d38df");
