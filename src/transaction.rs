...
use crate::circuit::{mint::Mint, output::Output, spend::Spend, spend_output::SpendOutput};
use crate::constants::MEMO_CHANGE_NOTE;
use crate::contract::{
    self, scalar_to_raw_bytes_le, AffineProofBytesLE, PlsAuthenticate, PlsAuthenticateAction,
    PlsFtTransfer, PlsMint, PlsMintAction, PlsNftTransfer, PlsOutput, PlsPublishNotesAction,
    PlsSpend, PlsSpendAction, PlsSpendOutput, PlsSpendSequence, PlsUnshieldedRecipient,
    PlsWithdraw, PlsWithdrawAction, ScalarBytes,
};
use crate::engine::Bls12;
use crate::engine::Scalar;
use crate::eosio::{
    pack, Action, Asset, Authorization, ExtendedAsset, Name, NameError, PackedAction, Symbol,
    Transaction,
};
use crate::keys::{FullViewingKey, SpendingKey};
use crate::note::nullifier::ExtractedNullifier;
use crate::note::{memo_to_bytes, ExtractedNoteCommitment, Note, NoteEx, Rseed};
...
use crate::spec::{extract_p, windowed_pedersen_commit};
...
fn fq_from_contract_raw_le(b_le: &[u8; 32], what: &'static str) -> Result<Fq, VerifyError> {
    ...
    Option::from(Fq::from_repr(repr)).ok_or(VerifyError::ScalarDecode(what))
}
#[inline]
...
impl fmt::Display for TransactionError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::InvalidChainIDLength => {
                write!(f, "invalid chain_id length (must equal 32 bytes)")
            }
            Self::IvkWallet => write!(f, "Read-Only Wallet (spending not possible)"),
            Self::AuthTokenContractAccount => write!(
                f,
                "For Auth tokens 'from' account must equal 'contract' account"
            ),
            ...
            Self::InvalidProtocolContractOrVaultContractOrAliasAuthority => write!(
                f,
                "protocol contract or vault contract or alias authority invalid"
            ),
...
fn select_ft_notes(
    ...
    for i in (0..unspent_notes.len()).rev() {
        if contract.eq(unspent_notes[i].note().contract()) &&               // same contract
            ...
            !unspent_notes[i].note().quantity().is_nft()
        // no nft or auth token
        {
...
pub fn resolve_ztransaction(
    wallet: &Wallet,
    fee_token_contract: &Name,
    fees: &HashMap<Name, Asset>,
    ztx: &ZTransaction,
) -> Result<ResolvedZTransaction, TransactionError> {
    if wallet.is_ivk() {
    ...
    let chain_id: [u8; 32] = chain_bytes
        .try_into()
    ...
    let mut fee = {
        let name = Name::from_string(&"begin").map_err(|_| TransactionError::FeeSymbol)?;
        fees.get(&name).ok_or(TransactionError::FeeSymbol)?.amount()
    };
    ...
    if ztx.add_fee {
        ...
        for rza in rztx.rzactions.iter_mut() {
            if rza.name == Name(14219329122852667392)
            // spend
            {
                let mut data: ResolvedSpendSequenceDesc = serde_json::from_value(rza.data.clone())?;

                if data.spend_output[0]
                    .note_a
                    .note()
                    .contract()
                    .eq(&fee_token_contract)
                    && data.spend_output[0].note_a.note().symbol().eq(fees
                        .values()
                        .next()
                        .ok_or(TransactionError::FeeSymbol)?
                        .symbol())
                {
                    ...
                    if data.output.len() == 0 {
                        ...
                        let notes_to_spend = select_fee_notes(
                            &mut unspent_notes,
                            &fees,
                            &fee_token_contract,
                            note_b.amount(),
                        ...
                        data.spend_output[0].unshielded_outputs.push((
                            Note::from_parts(
                                ...
                                ExtendedAsset::new(
                                    Asset::new(fee as i64, note_b.symbol().clone())
                                        .ok_or(TransactionError::FeeSymbol)?,
                                    note_b.contract().clone(),
                                ),
                                Rseed::new(&mut rng),
                                memo_to_bytes("fee"),
                            ),
                    ...
                    // change note is the last note in 'output' vector
                    {
                        // the change note is always the last shielded output
...
pub fn zsign_transaction(
    wallet: &Wallet,
    rztx: &ResolvedZTransaction,
    params: &HashMap<Name, Parameters<Bls12>>,
) -> Result<(Transaction, TxMeta), TransactionError> {
    if wallet.is_ivk() {
        Err(TransactionError::IvkWallet)?
    }
    ...
    while i < rztx.rzactions.len() {
        match rztx.rzactions[i].name {
            ...
            Name(10639630974360485888) => {
                ...
                while i < rztx.rzactions.len()
                    && rztx.rzactions[i].name == Name(10639630974360485888)
                {
                    let data: ResolvedMintDesc =
                        serde_json::from_value(rztx.rzactions[i].data.clone())?;
                        ...
                        mints.push(PlsMint {
                            ...
                            proof: {
                                ...
                                let proof = create_random_proof(instance, mint_params, &mut OsRng)
                                    .map_err(|e| {
                                        TransactionError::ProofSystem(format!("mint: {}", e))
                                    })?;
                                AffineProofBytesLE::try_from(proof)
                                    .map_err(|_| TransactionError::ProofBytes)?
                            },
                        });
                    }
                    ...
                    if data.publish_note {
                        ...
                        if recipient != wallet.default_address().unwrap().to_bech32m()? {
                            if meta.unpublished_notes.contains_key(&recipient) {
                                meta.unpublished_notes
                                    .get_mut(&recipient)
                                    .unwrap()
                                    .push(note_ct.clone());
                            } else {
                                meta.unpublished_notes
                                    .insert(recipient, vec![note_ct.clone()]);
                            }
                        }
                        if meta.unpublished_notes.contains_key(&"self".to_string()) {
                            meta.unpublished_notes
                                .get_mut(&"self".to_string())
                                .unwrap()
                                .push(note_ct);
                        } else {
                            meta.unpublished_notes
                                .insert("self".to_string(), vec![note_ct]);
                        }
                    }
            ...
            Name(14219329122852667392) => {
                ...
                let mut note_cts = vec![];
                while i < rztx.rzactions.len()
                    && rztx.rzactions[i].name == Name(14219329122852667392)
                {
                    let data: ResolvedSpendSequenceDesc =
                        serde_json::from_value(rztx.rzactions[i].data.clone())?;
                    ...
                    for (i, so) in data.spend_output.iter().enumerate() {
                        ...
                        if sister_path_and_root.is_none() {
                            Err(TransactionError::InvalidNote)?
                        }
                        let (sister_path, root) = sister_path_and_root.unwrap();

                        let value_spend = so.note_b.amount()
                            + so.unshielded_outputs
                                .iter()
                                .map(|uo| uo.0.amount())
                                .sum::<u64>();
                        let value_c = value_spend - so.note_b.amount();
                        let net_value = if so.note_a.note().amount() > value_spend {
                            so.note_a.note().amount() - value_spend
                        } else {
                            value_spend - so.note_a.note().amount()
                        };
                        ...
                        let cv_eq_gt = if so.note_a.note().amount() == value_spend {
                            2
                        } else {
                            0
                        } | if so.note_a.note().amount() > value_spend {
                            1
                        } else {
                            0
                        };
                        ...
                        spend_output.push(PlsSpendOutput {
                            root,
                            nf: ScalarBytes(
                                ExtractedNullifier::from(
                                    so.note_a.note().nullifier(&fvk.nk, so.note_a.position()),
                                )
                                .to_bytes(),
                            ),
                            ...
                            symbol: if value_c == 0 {
                                ...
                                so.note_a.note().symbol().clone()
                            },
                            contract: if value_c == 0 {
                                Name(0)
                            } else {
                                so.note_a.note().contract().clone()
                            },
                            cv_eq_gt,
                            proof: {
                                let instance = SpendOutput {
                                    ...
                                    unshielded_outputs_hash: {
                                        let mut state = Blake2s7rParams::new()
                                            .hash_length(32)
                                            .personal(&[0; 8])
                                            .to_state();
                                ...
                                let proof =
                                    create_random_proof(instance, spend_output_params, &mut OsRng)
                                        .map_err(|e| {
                                            TransactionError::ProofSystem(format!(
                                                "spend_output: {}",
                                                e
                                            ))
                                        })?;
                            ...
                            unshielded_outputs: so
                                .unshielded_outputs
                                .iter()
                                .map(|uo| PlsUnshieldedRecipient {
                                    amount: uo.0.amount(),
                                    account: uo.0.account(),
                                    memo: String::from_utf8(
                                        uo.0.memo()[0..{
                                            let len = uo.0.memo().iter().position(|&c| c == 0);
                                            if len.is_none() {
                                                512
                                            } else {
                                                len.unwrap()
                        ...
                        if !(so.note_a.note().symbol().raw() == 0
                            && so.unshielded_outputs.len() == 1)
                        {
                            let note_b_ct = {
                                ...
                                if recipient != wallet.default_address().unwrap().to_bech32m()? {
                                    if meta.unpublished_notes.contains_key(&recipient) {
                                        meta.unpublished_notes
                                            .get_mut(&recipient)
                                            .unwrap()
                                            .push(note_b_ct.clone());
                                    } else {
                                        meta.unpublished_notes
                                            .insert(recipient, vec![note_b_ct.clone()]);
                                    }
                                }
                                if meta.unpublished_notes.contains_key(&"self".to_string()) {
                                    meta.unpublished_notes
                                    ...
                                    meta.unpublished_notes
                                        .insert("self".to_string(), vec![note_b_ct]);
                                }
                            }
                        }
                    ...
                    for s in data.spend.iter() {
                        ...
                        spend.push(PlsSpend {
                            ...
                            nf: ScalarBytes(
                                ExtractedNullifier::from(
                                    s.note_a.note().nullifier(&fvk.nk, s.note_a.position()),
                                )
                                .to_bytes(),
                            ),
                            ...
                            proof: {
                                ...
                                let pr = create_random_proof(instance, spend_params, &mut OsRng)
                                    .map_err(|e| {
                                        TransactionError::ProofSystem(format!("spend: {}", e))
                                    })?;
                                AffineProofBytesLE::try_from(pr)
                                    .map_err(|_| TransactionError::ProofBytes)?
                            },
                        });
                    }
                        ...
                        output.push(PlsOutput {
                            ...
                            proof: {
                                ...
                                let pr = create_random_proof(instance, output_params, &mut OsRng)
                                    .map_err(|e| {
                                    TransactionError::ProofSystem(format!("output: {}", e))
                                })?;
                                AffineProofBytesLE::try_from(pr)
                                    .map_err(|_| TransactionError::ProofBytes)?
                            },
            ...
            Name(3941447159957795488) => {
                let data: ResolvedAuthenticateDesc =
                    serde_json::from_value(rztx.rzactions[i].data.clone())?;
            ...
            Name(12578297992662373760) => {
                let data: ResolvedPublishNotesDesc =
                    serde_json::from_value(rztx.rzactions[i].data.clone())?;
            ...
            Name(16407410437513019392) => {
                ...
                while i < rztx.rzactions.len()
                    && rztx.rzactions[i].name == Name(16407410437513019392)
                {
                    let data: ResolvedWithdrawDesc =
                        serde_json::from_value(rztx.rzactions[i].data.clone())?;
                ...
                tx.actions.push(Action {
                    ...
                    authorization: vec![rztx.alias_authority.clone()],
                    data: serde_json::to_value(PlsWithdrawAction { actions: withdraws })?,
                });
            }
...
#[inline]
fn eng_scalar_from_contract_raw_le(
    raw_le: &[u8; 32],
    what: &'static str,
    ...
    crate::engine::scalar_from_canonical_bytes(&canon).ok_or(VerifyError::ScalarDecode(what))
}
...
pub fn zverify_spend_transaction(
    tx: &Transaction,
    params: &HashMap<Name, Parameters<Bls12>>,
) -> Result<(), VerifyError> {
    let spend_output_params = params
        .get(&Name::from_string(&"spendoutput").unwrap())
        .ok_or(VerifyError::MissingParams("spendoutput"))?;
    let spend_params = params
        .get(&Name::from_string(&"spend").unwrap())
        .ok_or(VerifyError::MissingParams("spend"))?;
    let output_params = params
        .get(&Name::from_string(&"output").unwrap())
            ...
            Name(14219329122852667392) => {
                ...
                for a in data.actions.iter() {
                    for so in a.spend_output.iter() {
                        ...
                        let mut inputs = vec![];
                        inputs.push(
                            Scalar::try_from(so.root.clone())
                                .map_err(|_| VerifyError::ScalarDecode("root"))?,
                        );
                        inputs.push(
                            Scalar::try_from(so.nf.clone())
                                .map_err(|_| VerifyError::ScalarDecode("nf"))?,
                        );
                        inputs.push(
                            Scalar::try_from(a.scm.clone())
                                .map_err(|_| VerifyError::ScalarDecode("scm"))?,
                        );
                        inputs.push(
                            Scalar::try_from(so.cm_b.clone())
                                .map_err(|_| VerifyError::ScalarDecode("cm_b"))?,
                        );
                        ...
                        let proof = Proof::<Bls12>::try_from(so.proof.clone())
                            .map_err(|_| VerifyError::ProofBytes)?;
                        verify_proof(&pvk, &proof, &inputs)
                            .map_err(|_| VerifyError::ProofInvalid("spendoutput"))?;

                        if !((so.cv_eq_gt & 2) == 2) {
                            if (so.cv_eq_gt & 1) == 1 {
                                if spend_sum_begin {
                                    spend_sum = eng_scalar_from_contract_raw_point_le(
                                        &so.cv_net_u.0,
                                        &so.cv_net_v.0,
                                        "cv_net_(u/v)",
                                    )?
                                    ...
                                    spend_sum.add_assign(eng_scalar_from_contract_raw_point_le(
                                        &so.cv_net_u.0,
                                        &so.cv_net_v.0,
                                        "cv_net_(u/v)",
                                    )?);
                                ...
                                if output_sum_begin {
                                    output_sum = eng_scalar_from_contract_raw_point_le(
                                        &so.cv_net_u.0,
                                        &so.cv_net_v.0,
                                        "cv_net_(u/v)",
                                    )?
                                    ...
                                    output_sum.add_assign(eng_scalar_from_contract_raw_point_le(
                                        &so.cv_net_u.0,
                                        &so.cv_net_v.0,
                                        "cv_net_(u/v)",
                                    )?);
                    ...
                    for s in a.spend.iter() {
                        let mut inputs = vec![];
                        inputs.push(
                            Scalar::try_from(s.root.clone())
                                .map_err(|_| VerifyError::ScalarDecode("root"))?,
                        );
                        inputs.push(
                            Scalar::try_from(s.nf.clone())
                                .map_err(|_| VerifyError::ScalarDecode("nf"))?,
                        );
                        inputs.push(
                            Scalar::try_from(a.scm.clone())
                        ...
                        let proof = Proof::<Bls12>::try_from(s.proof.clone())
                            .map_err(|_| VerifyError::ProofBytes)?;
                        verify_proof(&pvk, &proof, &inputs)
                            .map_err(|_| VerifyError::ProofInvalid("spend"))?;

                        let p = eng_scalar_from_contract_raw_point_le(
                            &s.cv_u.0, &s.cv_v.0, "cv_(u/v)",
                        )?;
                        if spend_sum_begin {
                    ...
                    for o in a.output.iter() {
                        ...
                        inputs.push(
                            Scalar::try_from(o.cm.clone())
                                .map_err(|_| VerifyError::ScalarDecode("cm"))?,
                        );
                        ...
                        let proof = Proof::<Bls12>::try_from(o.proof.clone())
                            .map_err(|_| VerifyError::ProofBytes)?;
                        verify_proof(&pvk, &proof, &inputs)
                            .map_err(|_| VerifyError::ProofInvalid("output"))?;

                        let p = eng_scalar_from_contract_raw_point_le(
                            &o.cv_u.0, &o.cv_v.0, "cv_(u/v)",
                        )?;
                        if output_sum_begin {
...
pub fn zsign_transfer_and_mint_transaction(
    ...
    for desc in mint_zactions.iter() {
        let n = Note::from_parts(
            ...
            {
                let mut memo_bytes = [0; 512];
                memo_bytes[0..min(512, desc.memo.len())]
                    .copy_from_slice(&desc.memo.as_bytes()[0..min(512, desc.memo.len())]);
                memo_bytes
            },
        );
    ...
    let fee_m = {
        let name = Name::from_string(&"mint").map_err(|_| TransactionError::FeeSymbol)?;
        fees.get(&name).ok_or(TransactionError::FeeSymbol)?.amount()
    };
    ...
    for rm in resolved_mints.iter() {
        ...
        let note_ct = {
            ...
            let ne = NoteEncryption::new(
                Some(FullViewingKey::from_spending_key(&sk).ovk),
                rm.note.clone(),
            );
            let encrypted_note = TransmittedNoteCiphertext {
                epk_bytes: epk.to_bytes().0,
...
pub fn create_auth_token(
    ...
    contract: Name,
    recipient: Address,
) -> Result<HashMap<String, Vec<String>>, TransactionError> {
    if wallet.is_ivk() {
        Err(TransactionError::IvkWallet)?
    }
    let fvk = FullViewingKey::from_spending_key(&wallet.spending_key().unwrap());
    ...
    let n = Note::from_parts(
        ...
        {
            let mut memo_bytes = [0; 512];
            memo_bytes[0..min(512, seed.len())]
                .copy_from_slice(&seed.as_bytes()[0..min(512, seed.len())]);
            memo_bytes
        },
    );
...
#[cfg(test)]
mod tests {
    use super::{insert_vars, zsign_transaction, Name};
    use crate::engine::Bls12;
    use crate::{
        address::Address,
        eosio::{Asset, Authorization, ExtendedAsset},
        keys::{FullViewingKey, SpendingKey},
        note::{Note, Rseed},
        note_encryption::{
            derive_esk, ka_derive_public, NoteEncryption, TransmittedNoteCiphertext,
        },
        transaction::{resolve_ztransaction, zverify_spend_transaction, ZTransaction},
        wallet::Wallet,
    };
    ...
    fn test_insert_auth_into_memo() {
        ...
        let auth_tokens = vec![
            Note::from_parts(
                ...
                Address::dummy(&mut rng),
                Name(0),
                ExtendedAsset::new(Asset::from_string(&"0").unwrap(), Name(9999)),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
        ...
        m.insert(
            Name::from_string(&"begin").unwrap(),
            Asset::from_string(&"5.0000 ZEOS").unwrap(),
        );
        m.insert(
            Name::from_string(&"mint").unwrap(),
            Asset::from_string(&"1.0000 ZEOS").unwrap(),
        );
    ...
    fn test_mint_auth_publish_withdraw() {
        ...
        fees.insert(
            Name::from_string(&"spendoutput").unwrap(),
            Asset::from_string(&"1.0000 ZEOS").unwrap(),
        );
        fees.insert(
            Name::from_string(&"spend").unwrap(),
            Asset::from_string(&"1.0000 ZEOS").unwrap(),
        );
        fees.insert(
            Name::from_string(&"output").unwrap(),
            Asset::from_string(&"1.0000 ZEOS").unwrap(),
        );
        fees.insert(
            Name::from_string(&"authenticate").unwrap(),
            Asset::from_string(&"1.0000 ZEOS").unwrap(),
        );
        fees.insert(
            Name::from_string(&"publishnotes").unwrap(),
            Asset::from_string(&"1.0000 ZEOS").unwrap(),
        );
    ...
    fn test_spend() {
        ...
        let notes = vec![
            Note::from_parts(
                ...
                Name(0),
                ExtendedAsset::from_string(&"7.0000 ZEOS@thezeostoken").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
            ...
            Note::from_parts(
                ...
                w.default_address().unwrap(),
                Name(0),
                ExtendedAsset::from_string(&"10.0000 EOS@thezeostoken").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
                ...
                w.default_address().unwrap(),
                Name(0),
                ExtendedAsset::from_string(&"5.0000 EOS@thezeostoken").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
                ...
                w.default_address().unwrap(),
                Name(0),
                ExtendedAsset::from_string(&"4.0000 EOS@thezeostoken").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
            ...
            Note::from_parts(
                ...
                w.default_address().unwrap(),
                Name(0),
                ExtendedAsset::from_string(&"3.0000 EOS@thezeostoken").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
            ...
            Note::from_parts(
                ...
                w.default_address().unwrap(),
                Name(0),
                ExtendedAsset::from_string(&"2.0000 EOS@thezeostoken").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
            Note::from_parts(
            ...
            Note::from_parts(
                ...
                w.default_address().unwrap(),
                Name(0),
                ExtendedAsset::from_string(&"12345678987654321@atomicassets").unwrap(),
                Rseed::new(&mut rng),
                [0; 512],
            ),
        ];
        ...
        params.insert(
            Name::from_string(&"mint").unwrap(),
            Parameters::<Bls12>::read(f, false).unwrap(),
        );
    ...
    fn test_hash() {
        ...
        let note = Note::from_parts(
            ...
            Address::dummy(&mut rng),
            Name(0),
            ExtendedAsset::new(Asset::new(0, Symbol(0)).unwrap(), Name(0)),
            Rseed::new(&mut rng),
            {
                let memo = "this is a memo string".to_string();
                let mut memo_bytes = [0; 512];
                memo_bytes[0..min(512, memo.len())]
                    .copy_from_slice(&memo.as_bytes()[0..min(512, memo.len())]);
                memo_bytes
            },
        );
        println!("{:?}", note.memo());
        ...
        state.update(
            &note.memo()[0..{
                let len = note.memo().iter().position(|&c| c == 0);
                if len.is_none() {
                    512
                } else {
                    len.unwrap()
    ...
    fn test_zsign_transfer_and_mint_transaction() {
        ...
        let x = zsign_transfer_and_mint_transaction(
            &mint_zactions,
            &Authorization {
                actor: Name::from_string(&"thezeosalias").unwrap(),
                permission: Name::from_string(&"public").unwrap(),
            },
            &Authorization {
                actor: Name::from_string(&"mschoenebeck").unwrap(),
                permission: Name::from_string(&"active").unwrap(),
            },
            Name::from_string(&"zeos4privacy").unwrap(),
